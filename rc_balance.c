	...
	while ((c = getopt(argc, argv, "i:")) != -1){
		...
		case 'i': // input option
			if(!strcmp("dsm", optarg)) {
				...
				m_input_mode = NONE;
			} else if(!strcmp("keycontrols", optarg)){
				m_input_mode = KEYCONTROLS;
			} else {
				__print_usage();
				return -1;
			}
...
void* __setpoint_manager(__attribute__ ((unused)) void* ptr)
{
	...
	int i, j = 0,  ch, chan, stdin_timeout = 0; // for stdin & KEYCONTROLS input
	char in_str[11];
	#define  turn_speed	3
	#define  drive_speed	20
	#define  turn_delay	1
	#define  straight_delay	1
	...
	while(rc_get_state()!=EXITING){
		...
		case DSM:
			if(rc_dsm_is_new_data()){
				...
				rc_saturate_double(&drive_stick,-2,1);
...
/*			while((ch=getchar()) != EOF){
				...
				while (j <= 3){
                                                setpoint.phi_dot = drive_speed;
                                                sleep(straight_delay);
                                                setpoint.phi_dot = 0;
                                                setpoint.gamma_dot = turn_speed;
                                                sleep(turn_delay);
                                                setpoint.gamma_dot = 0;
                                                usleep(500);
                                                j++;
				...
				while (j <= 2){
                                                ...
                                                sleep(straight_delay);
                                                setpoint.phi_dot = 0;
                                                setpoint.gamma_dot = turn_speed+1;
                                                sleep(turn_delay);
                                                setpoint.gamma_dot = 0;
                                                usleep(500);
                                                j++;
				...
				//drive in circle
				setpoint.phi_dot = drive_speed;
				setpoint.gamma_dot = turn_speed+2;
				}
*/
			if(kbhit()){
				switch(fgetc(stdin)){
				...
				break;
				case 's':
				//increase left turning velocity
				setpoint.gamma_dot++;
				break;
				case 'd':
				//decrease left turning velocity
				setpoint.gamma_dot--;
				break;
				case '1':
				//Square pattern
